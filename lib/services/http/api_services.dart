...
import 'package:sports_management/services/init_headers.dart';
import 'package:sports_management/utils/staticNamesServices.dart';
...
class ApiServices {
  ...
  Future<Result<Init>> init(String businessId) async {
    ...
    var uri = url("${MyUtils.type}${StaticNamesPath.init.path}", queryParameters: params);

    return httpCall((client) => client.get(uri, headers: headers),
        parseJson: (json) => Init.fromJson(json));
  }
  Future<Result<InventoryModel>> inventory({required Map<String, String> params}) async {
    var headers = {
      HttpHeaders.contentTypeHeader: ContentType.json.toString(),
    };

    var uri = url("${MyUtils.type}${StaticNamesPath.inventory.path}", queryParameters: params);

    return httpCall((client) => client.get(uri, headers: headers),
        parseJson: (json) => InventoryModel.fromJson(json));
  }

  Future<Result<CollectChannel>> getBanks({required Map<String, String> params}) async {
    var headers = {
      HttpHeaders.contentTypeHeader: ContentType.json.toString(),
    };

    var uri = url("${MyUtils.type}${StaticNamesPath.banks.path}", queryParameters: params);

    return httpCall((client) => client.get(uri, headers: headers),
        parseJson: (json) => CollectChannel.fromJson(json));
  }

  Future<Result<CurrencyRate>> getRate({required Map<String, String> params, required Map<String, dynamic> body}) async {
    var headers = {
      HttpHeaders.contentTypeHeader: ContentType.json.toString(),
    };

    var uri = url("${MyUtils.type}${StaticNamesPath.rate.path}", queryParameters: params);

    return httpCall((client) => client.post(uri, headers: headers, body: jsonEncode(body)),
        parseJson: (json) => CurrencyRate.fromJson(json));
  }

  Future<Result<DetailsReportModel>> getDetails({required Map<String, String> params, required Map<String, dynamic> body}) async {
    var headers = {
      HttpHeaders.contentTypeHeader: ContentType.json.toString(),
    };

    var uri = url("${MyUtils.type}${StaticNamesPath.details.path}", queryParameters: params);
  ...
  Future<Result<AccessTokenResponse>> authorize(String auth) {
    ...
    return httpCall((client) => client.post(uri, headers: headers, body: jsonEncode(body)),
            parseJson: (json) => Message.fromJson(json)).then((value) => Result(value.success, value.obj, value.error,
            value.stackTrace, value.errorMessage, value.obj));
  }
