...
class _ChangePasswordState extends State<ChangePassword> {
  ...
  Future<Result> sendChangePassword() async {
    Map<String, dynamic> body = {
      "password":
          Cryptom.encrypt(oldPasswordController.text, MyUtils.publicKey),
      "new_password":
          Cryptom.encrypt(newPasswordController.text, MyUtils.publicKey),
    };
  ...
  buttonAction() async {
    ...
    if (sendChange.success) {
      ...
      showDialog(
          ...
          builder: (BuildContext context) => PopScope(
              canPop: false,
              child: Alerts.error(sendChange.errorMessage ?? "Error",
                  () => Navigator.pop(context))));
    }
    setState(() {
      loading = false;
